        ...
        for i in range(self.N_fits):
            ...
            for index, (lname, wave, norm, shift, slow, shigh) in enumerate(zip(line_names,line_wave,line_norms,line_shifts,sigma_lows,sigma_highs)):
                ...
                line_parinfo = [{'name':'Skew_'+str(index), 'limits': (-10., 10.), 'init_value': 0., 'fixed': True}, 
                                {'name':'Sigma_'+str(index), 'limits': (slow, shigh), 'init_value': (slow+shigh)/2.+(-1)**(index)*index*0.9*(shigh-slow)/(2.*len(line_names))+np.random.normal(0, 1E-4), 'fixed':False}, 
                                {'name':'Norm_'+str(index), 'limits': (0, 1E4), 'init_value': norm+index*np.random.normal(0, 1E-4), 'fixed':False}, 
                                {'name':'Wavelength_'+str(index), 'limits': (wave+self.globalLineShift-shift, wave+self.globalLineShift+shift), 'init_value':wave+(-1)**(index)*index*np.random.normal(0, 1E-4), 'fixed':False}]

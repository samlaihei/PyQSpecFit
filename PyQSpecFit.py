...
class PyQSpecFit():
    ...
    def Fit(self, dataFile, runName=None):
        ...
        for i in range(self.N_fits):
            ...
            result = minimize(self.conti_residuals, params, args=[(lams, qso_resid, eflux, self.contiWindow)],
                              method='least_squares', calc_covar=False, xtol=1E-8, ftol=1E-8, gtol=1E-8,
                              max_nfev=100 * (len(params) + 1))
            ...
            for index, (lname, wave, norm, shift, slow, shigh) in enumerate(
                    zip(line_names, line_wave, line_norms, line_shifts, sigma_lows, sigma_highs)):
                # Line Parameters: Skew, Scale, Norm, Central Wavelength
                line_parinfo = [{'name': 'Skew_' + str(index), 'limits': (-10., 10.), 'init_value': 0., 'fixed': True},
                                {'name': 'Sigma_' + str(index), 'limits': (slow, shigh),
                                 'init_value': (slow + shigh) / 2. + (-1) ** (index) * index * 0.9 * (shigh - slow) / (
                                             2. * len(line_names)) + np.random.normal(0, 1E-4), 'fixed': False},
                                {'name': 'Norm_' + str(index), 'limits': (0, 1E4),
                                 'init_value': norm + index * np.random.normal(0, 1E-4), 'fixed': False},
                                {'name': 'Wavelength_' + str(index),
                                 'limits': (wave + self.globalLineShift - shift, wave + self.globalLineShift + shift),
                                 'init_value': wave + (-1) ** (index) * index * np.random.normal(0, 1E-4),
                                 'fixed': False}]
            ...
            for pars in init_parinfo:
                if 'Wavelength' in pars['name']:
                    params.add(pars['name'], value=np.log(pars['init_value']), min=np.log(pars['limits'][0]),
                               max=np.log(pars['limits'][1]), vary=not pars['fixed'])
                else:
                    params.add(pars['name'], value=pars['init_value'], min=pars['limits'][0], max=pars['limits'][1],
                               vary=not pars['fixed'])
            ...
            if self.clipSigma > 0:
                ...
                xx, yy, e_yy = self.sigma_mask_buffer(self.clipBoxWidth, self.clipSigma, xx, yy, e_yy,
                                                      self.clipBufferWidth)

            result = minimize(self.residual_line, params, args=[(xx, yy, e_yy, self.lineWindow, init_parinfo)],
                              method='least_squares', calc_covar=False, xtol=1E-8, ftol=1E-8, gtol=1E-8,
                              max_nfev=100 * (len(params) + 1))
            fitted_params = result.params
            fitted_values = fitted_params.valuesdict()
    ...
    def evalLineProperties(self, lineFile, fitFile, redshift, monoLumAngstrom=3000.,
        ...
        contips = np.transpose(
            [pdata[i].to_numpy().reshape(int(len(pdata[i].to_numpy()) / N_gauss), N_gauss) for i in conti_header])[0]
        ...
        for index, (norms, wavs, fwhms, contip, rescale_facs) in enumerate(
                zip(line_norms, line_wavs, line_FWHM, contips, rescale)):
            if Fe_cycle:
            ...
            # Assumes flux is in units of erg/s/cm2/Angstrom #
            lum = self.eval_PL(contip, [monoLumAngstrom]) * rescale_facs[
                0] * 4 * np.pi * dl.value ** 2 * monoLumAngstrom * (1 + redshift)
            ...
            temp_res_props[2].append(
                (vac_wav - self.general_med_wav(lams, tot_line)) / vac_wav * con.c / 1000.)  # Blueshift
            ...
            temp_res_props[5].append(np.log10(self.calc_line_flux(lams, tot_line) * 4 * np.pi * dl.value ** 2 * (
                        1 + redshift)))  # Integrated Luminosity
    ...
    def plotLineFits(self, data_ax, resid_ax, lineFile, dataFile, fitFile, redshift,
        ...
        for line_index, (norm, wav, fwhm, rescale_fac) in enumerate(zip(line_norms, line_wavs, line_FWHM, rescale)):
            ...
            current_inset_window = np.clip(lams[np.where(temp_line > 0.01 * np.nanmax(temp_line))], plotWindow[0],
                                           plotWindow[1])
            ...
            data_ax.plot(lams[np.logical_and(lams > current_inset_window[0], lams < current_inset_window[1])],
                         (conti_profile + temp_line)[
                             np.logical_and(lams > current_inset_window[0], lams < current_inset_window[1])],
                         '--', c=dashed_color)
        ...
        current_inset_window = np.clip(lams[np.where(tot_line > 0.01 * np.nanmax(tot_line))], plotWindow[0],
                                       plotWindow[1])
        if len(current_inset_window) > 0:
            current_inset_window = np.array([np.nanmin(current_inset_window), np.nanmax(current_inset_window)])
        else:
            current_inset_window = np.copy(plotWindow)
        data_ax.tick_params(which='both', axis='both', direction='in', bottom=True, top=True, right=True,
        ...
        data_ax.set_ylim(np.clip(data_ax.get_ylim(), 0.25 * np.min(((conti_profile + tot_line)[tot_line > 0.01])),
                                 1.5 * np.max((conti_profile + tot_line)[tot_line > 0.01])))
        ...
        data_ax.plot(lams[np.logical_and(lams > plotWindow[0], lams < plotWindow[1])],
                     ...
                     c='b')
        data_ax.plot(lams[tot_line > 0.01][
                         np.logical_and(lams[tot_line > 0.01] > plotWindow[0], lams[tot_line > 0.01] < plotWindow[1])],
                     (conti_profile + tot_line)[tot_line > 0.01][
                         np.logical_and(lams[tot_line > 0.01] > plotWindow[0], lams[tot_line > 0.01] < plotWindow[1])],
                     c='r')
        data_ax.plot(lams[np.logical_and(lams > current_inset_window[0], lams < current_inset_window[1])],
                     (conti_profile + tot_line)[
                         np.logical_and(lams > current_inset_window[0], lams < current_inset_window[1])],
                     c='r', label='Line')
        ...
        if resid_ax != None:
            resid_ax.tick_params(which='both', axis='both', direction='in', bottom=True, top=True, right=True,
                                 labelleft=True)
            resid_ax.plot(lams[np.logical_and(lams > current_inset_window[0], lams < current_inset_window[1])],
                          ((data_flux - conti_profile - tot_line) / data_eflux)[
                              np.logical_and(lams > current_inset_window[0], lams < current_inset_window[1])],
                          c='r', zorder=3)
            ...
            resid_ax.plot(lams[np.logical_and(lams > plotWindow[0], lams < plotWindow[1])],
                          ((data_flux - conti_profile - tot_line) / data_eflux)[
                              np.logical_and(lams > plotWindow[0], lams < plotWindow[1])])

            resid_ax.plot(lams[np.logical_and(lams > plotWindow[0], lams < plotWindow[1])],
                          [0 for i in lams[np.logical_and(lams > plotWindow[0], lams < plotWindow[1])]],
                          c='k', zorder=4)
            resid_ax.set_ylim(np.clip(resid_ax.get_ylim(), -15, 15))
            resid_ax.plot(atm_lams_plot,
                          [0.93 * (resid_ax.get_ylim()[1] - resid_ax.get_ylim()[0]) + resid_ax.get_ylim()[0] for i in
                           atm_lams_plot],
    ...
    def calc_fwhm(self, xx, line_yy, line_center):
        res = specutils_fwhm(
            Spectrum1D(spectral_axis=xx * u.angstrom, flux=line_yy * u.Jy)).value / line_center * con.c / 1000.
        return res
    ...
    def Balmer_conti(self, pp, xval):
        ...
        if pp[0] > 0:
            ...
            bbflux = 1E-8 * bb(xval * u.AA) * (np.pi * u.sr) * (con.c * 1E10 * u.AA * u.Hz) / (
                        xval * u.AA) ** 2  # in units of ergs/cm2/s/A
    ...
    def eval_conti_all(self, p, xx):
        ...
        f_Fe_MgII = self.Fe_flux(p[2:5], xx, self.fe_uvs[self.Fe_uv_ind],
                                 self.init_fe_uv_fwhms[self.Fe_uv_ind])  # iron flux for MgII line region
        f_Fe_Balmer = self.Fe_flux(p[5:8], xx, self.fe_ops[self.Fe_opt_ind],
                                   self.init_fe_op_fwhms[self.Fe_opt_ind])  # iron flux for balmer line region
    ...
    def out_line_res(self, lams, p, line_names):
        ...
        for i in range(num_lines):
            ...
            if self.dataOut:
                ...
                data_row = [self.runName, line_names[i]] + [self.norm_median] + list(current_line_params) + list(
                    self.conti_bestfit)
    ...
    def readLineFile(self, lineFile, lineCompInd):
        ...
        line_indices = []
        for ind, (name, c_wav) in enumerate(
                zip(line_list_pdata['Name'].to_numpy(), line_list_pdata['Central Wavelength'])):
